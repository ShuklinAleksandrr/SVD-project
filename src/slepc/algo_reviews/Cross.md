...
В математике обобщённая матрица перестановок — это матрица с тем же набором ненулевых элементов, то есть в каждой строке и каждом столбце есть ровно одна ненулевая запись. В отличие от матрицы перестановок, где ненулевая запись должна быть равна 1, в обобщённой матрице перестановок ненулевая запись может быть любым ненулевым значением.
...
Плохо обусловленная матрица - матрица, у которой небольшие изменения в элементах матрицы могут привести к значительным изменениям в решении системы линейных уравнений или при работе с матрицей. В результате, методы решения таких систем, такие как метод Гаусса, могут давать ошибочные результаты из-за ошибок округления.

Примеры неявных матриц: 
1. Разреженные матрицы
Многие задачи требуют работы с разреженными матрицами. Разреженные матрицы - такие, у которых большинство элементов равны нулю. Вместо того чтобы хранить всю матрицу целиком, используют специальные структуры данных, такие как списки смежностей или формат COO (координатный формат), которые хранят только ненулевые элементы вместе с их индексами. Таким образом, матрица становится неявной, поскольку большая часть её данных не хранится явно.
...
Квантование матриц - это процесс преобразования коэффициентов яркости и цветности для достижения большей степени сжатия в однородных областях изображения. Это достигается путем логарифмического управления длиной шага, что упрощает распределение битрейта кодером и вычисление обратной длины квантования.

#### Квантование матриц

##### Прямое квантование

При прямом квантовании значение сигнала $x$ квантуется до ближайшего уровня квантования $q$. Это можно выразить следующим образом:

$$
x_q = \left\lfloor \frac{x}{Q} + \frac{1}{2} \right\rfloor
$$

где:
- $x_q$ – квантованное значение,
- $Q$ – шаг квантования,
- $\lfloor \cdot \rfloor$ – операция округления вниз.

##### Деквантование

Для восстановления исходного значения используется обратная формула деквантования:

$$
\hat{x} = Q \cdot x_q
$$

где $\hat{x}$ – восстановленное значение.

##### Матрица квантования

Матрицы квантования используются для оптимизации процесса квантования в разных частотных диапазонах. Например, в JPEG используется следующая формула для создания матрицы квантования:

$$
Q_{ij} = S_f \times B_i \times C_j
$$

где:
- $S_f$ – базовый коэффициент масштабирования,
- $B_i$, $C_j$ – весовые коэффициенты для строк и столбцов соответственно.

Эти веса могут быть выбраны так, чтобы минимизировать видимые искажения после квантования.

##### Логарифмическое управление длиной шага

Для адаптивного изменения шага квантования может использоваться логарифмическая шкала:

$$
L(x) = \log_2\left(\frac{|x|}{Q}\right)
$$

где $L(x)$ – длина шага квантования для значения $x$.

##### Частотно-оптимизированные матрицы масштабирования

Частотно-оптимизированные матрицы выбираются на основе анализа человеческого зрения и восприятия различных частотных компонентов изображения. Они помогают оптимизировать квантование таким образом, чтобы минимально влиять на качество восприятия человеком. Эти матрицы обычно представлены в виде таблиц значений, где каждый элемент соответствует определенному уровню квантования.


3. Алгоритмическое задание матриц
Алгоритмическое задание матриц - это процесс, который генерирует элементы матрицы, с помощью определенного алгоритма. Иногда матрицы могут задаваться алгоритмами, которые генерируют элементы по мере необходимости. Например, если матрица является результатом некоторого процесса, её элементы могут вычисляться динамически, а не храниться в памяти заранее.
Пример: Матрица Хилберта — это квадратная матрица, элементы которой определяются по формуле $H_{i,j} = \frac{1}{i+j-1} $

4. Использование функций для генерации элементов.
Генерация элементов матрицы с использованием функции - это процесс, при котором значение элемента вычисляет с использованием какой-то функции и индексов элемента матрицы. Такой подход особенно полезен, когда необходимо работать с очень большими матрицами или с бесконечными последовательностями.
...
Инверсивное преобразование - это математическая операция, которая меняет местами элементы множества или изменяет направление некоторых параметров. В геометрии инверсия - это конформное преобразование евклидовой плоскости или пространства. Она также может означать переворачивание смысла, замену "белого" "черным". В физике инверсивная трансформация может включать изменение типа проводимости в полупроводниках или изменение направления магнитного поля Земли. 
...
$\sigma = \sqrt{|\lambda|}$, где $\lambda$ - собственное значение изначальной матрицы, $\sigma$ - сингялурное соотвествующее значение.
...
#### Функция SVDSolve_Cross - Вычисляет сингулярные значения матрица через собственные значения матрицы. 
...
if (!ptype) PetscCall(EPSSetProblemType(cross->eps,EPS_GHEP));
```
Если тип проблемы не был задан, устанавливается тип EPS_GHEP (Обобщенный эрмитовый тип).
4) 
```
else if (svd->ishyperbolic && svd->swapped) {
...
```
Проверяется, явная ли матрица явной и используется ли метод Крылова-Шура (isks), и при этом не используется инверсивное преобразование (!issinv) 
```
        PetscCall(STSetType(st,STSINVERT));
        PetscCall(EPSSetTarget(cross->eps,0.0));
        which = EPS_TARGET_REAL;
...
PetscOptionsHeadBegin(PetscOptionsObject,"SVD Cross Options");
  ...
  PetscOptionsHeadEnd();
```
Предоставляет пользователю возможность выбрать использование явной матрицы. Если выбрано использование явной матрицы, то вызывается функция SVDCrossSetExplicitMatrix, которая устанавливает режим работы с явной матрицей.
```
if (!cross->eps) PetscCall(SVDCrossGetEPS(svd,&cross->eps));
  if (!cross->explicitmatrix && !cross->usereps) {
    /* use as default an ST with shell matrix and Jacobi */
