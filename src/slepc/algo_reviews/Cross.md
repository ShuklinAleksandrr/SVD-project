...
#### Функция SVDSolve_Cross - Решает проблему сингулярного значения (TODO что такое проблема сингулярного значения? соб). 
...
Примеры неявных матриц: (TODO перенести в введение)
1. Разреженные матрицы
Многие задачи требуют работы с разреженными матрицами, у которых большинство элементов равны нулю. (TODO исправить на Разреженные матрицы - такие, у которых большинство элементов равны нулю) Вместо того чтобы хранить всю матрицу целиком, используют специальные структуры данных, такие как списки смежностей или формат COO (координатный формат), которые хранят только ненулевые элементы вместе с их индексами. Таким образом, матрица становится неявной, поскольку большая часть её данных не хранится явно.

2. Квантование матриц (TODO определение квантования матрицы (представление матрицы в сжатом виде, путем ограниченяи колва бит на элемент))
...
3. Алгоритмическое задание матриц (TODO то же что в предыдущем)
...
4. Использование функций для генерации элементов (TODO то же что в предыдущем)
...
if (!ptype) PetscCall(EPSSetProblemType(cross->eps,EPS_GHEP));
```
Если тип проблемы не был задан, устанавливается тип EPS_GHEP (обобщенное собственное значение проблема (TODO непонятно что это такое, нужно подробнее)).
4) 
```
else if (svd->ishyperbolic && svd->swapped) {
...
```
Проверяется, явная ли матрица явной и используется ли метод Крылова-Шура (isks), и при этом не используется инвертированное преобразование (!issinv) (TODO что такое инвертированнное преобразование, в Введение)
```
        PetscCall(STSetType(st,STSINVERT));
        PetscCall(EPSSetTarget(cross->eps,0.0));
        which = EPS_TARGET_REAL;
...
PetscOptionsHeadBegin(PetscOptionsObject,"SVD Cross Options");
  ...
  PetscOptionsHeadEnd();
```
Предоставляет пользователю возможность выбрать использование явной матрицы вместо шельковой (TODO что такое шельковая матрица, в Введение) матрицы через интерфейс опций. Если выбрано использование явной матрицы, то вызывается функция SVDCrossSetExplicitMatrix, которая устанавливает режим работы с явной матрицей.
```
if (!cross->eps) PetscCall(SVDCrossGetEPS(svd,&cross->eps));
  if (!cross->explicitmatrix && !cross->usereps) {
    /* use as default an ST with shell matrix and Jacobi */
